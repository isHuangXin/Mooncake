...
// FLAT_MEMORY: unlike RuntimeAccelerators(), this never initializes CUDA.
// cuPointerGetAttributes can query device/managed/registered or ordinary host
// pointers without creating a context. Any driver/query failure is unknown,
// never Host. Keep the driver loaded for the lifetime of the function pointer.
#ifdef USE_CUDA
...
bool SsdFetchHostClassificationAvailable() {
    ...
    // These transports accept device memory without a Store classifier.
    return false;
#endif
...
// FLAT_MEMORY: managed/device pointers must never enter the Host-only totals.
device::MemoryKind RealClient::ssd_fetch_memory_kind(const void* ptr) const {
    ...
    // The multi-attribute driver query returns zero attributes for ordinary
    // unregistered Host memory (as opposed to the single-attribute API).
...
RealClient::batch_get_into_offload_object_internal(
    ...
    // FLAT_MEMORY: account once here for ordinary and multi-buffer callers.
    // A mixed Host/device owner sub-batch is excluded in its entirety.
    ...
    // FLAT_MEMORY: exclude setup/classification and include owner RPC + future.
    ...
    // FLAT_MEMORY: end_time follows BatchGetOffloadObject's future->get().
    // Do not include buffer release RPC or claim the outer checksum has passed.
    fetch_attempt.Complete(fetch_bytes, end_time);
    return {};
}
...
// FLAT_MEMORY: short-lock coherent snapshot; no reset, I/O, waits or GPU probe.
