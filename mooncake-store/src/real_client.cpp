...
std::string RealClient::get_storage_io_stats() {
    ...
    const bool available =
        file_storage_ && file_storage_->SupportsNativeIOMetrics();
    output += std::string(",\"ssd_io_available\":") +
    ...
    output += ",\"ssd_usage_scope\":\"bucket metadata bytes, not allocated "
              "filesystem blocks\"}";
    return output;
}
...
int RealClient::start_http_server(int port) {
    ...
    http_server_->set_http_handler<POST>(
        ...
        [this](coro_http_request &req, coro_http_response &resp) {
            ...
            auto parsed =
                std::from_chars(raw.data(), raw.data() + raw.size(), window_id);
            ...
            auto parsed_pid = std::from_chars(
                raw_pid.data(), raw_pid.data() + raw_pid.size(), owner_pid);
            if (parsed_pid.ec != std::errc{} ||
                parsed_pid.ptr != raw_pid.data() + raw_pid.size() ||
                owner_pid != static_cast<uint64_t>(getpid())) {
                resp.set_status_and_content(status_type::conflict,
                                            "I/O owner does not match");
                return;
            }
            if (!StorageIOMetric::Instance().EndWindow(window_id)) {
                resp.set_status_and_content(status_type::conflict,
                                            "I/O window does not match");
                return;
            }
            resp.add_header("Content-Type", "application/json");
            resp.set_status_and_content(status_type::ok, get_storage_io_stats());
