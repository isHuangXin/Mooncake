...
// CPU-only substitutes preserve the real ordinary/multi-buffer routing logic.
   ...
   public:
    ...
    std::vector<tl::expected<QueryResult, ErrorCode>> BatchQuery(
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            ...
            replica.descriptor_variant = LocalDiskDescriptor{{0, 1}, 8, "owner"};
...
TEST_F(ClientMetricsTest, IoSnapshotIsCoherentCumulativeAndPerInstance) {
    ...
    for (int i = 0; i < 2; ++i) {
        const auto snapshot = stats.Snapshot(true);
    ...
    writer.join();
    stats.MarkUnclassifiable();
    EXPECT_TRUE(stats.Snapshot(true).capabilities.empty());
    EXPECT_FALSE(stats.Snapshot(true).ssd_to_host_fetch);
}

TEST_F(ClientMetricsTest, IoSnapshotBackendCapabilityAndEpoch) {
    DummyClient dummy;
    const auto unsupported = dummy.get_io_stats_snapshot();
    EXPECT_EQ(unsupported.schema_version, 1);
    EXPECT_TRUE(unsupported.instance_id.empty());
    EXPECT_TRUE(unsupported.capabilities.empty());
    EXPECT_FALSE(unsupported.ssd_to_host_fetch);
    RealClient first, second;
    const auto snapshot = first.get_io_stats_snapshot();
    EXPECT_FALSE(snapshot.instance_id.empty());
    EXPECT_NE(snapshot.instance_id, second.get_io_stats_snapshot().instance_id);
    EXPECT_EQ(snapshot.instance_id, first.get_io_stats_snapshot().instance_id);
}
...
TEST_F(ClientMetricsTest, IoFetchOrdinaryAndMultiBufferShareAccounting) {
    ...
    ASSERT_TRUE(real.get_io_stats_snapshot().ssd_to_host_fetch);
    char a[8] = {}, b[8] = {};
    ...
    ASSERT_TRUE(multi[0]);
    ASSERT_TRUE(multi[1]);
    const auto second = *real.get_io_stats_snapshot().ssd_to_host_fetch;
...
TEST_F(ClientMetricsTest, IoFetchWaitsForTransferAndExcludesFailures) {
    ...
    const auto pending = *real.get_io_stats_snapshot().ssd_to_host_fetch;
    ...
    client->during_transfer = {};
    const auto success = *real.get_io_stats_snapshot().ssd_to_host_fetch;
    ...
    requester->ttl_ms = 0;
    auto expired = real.batch_get_into_offload_object_internal("owner", objects);
    ...
    const auto after = *real.get_io_stats_snapshot().ssd_to_host_fetch;
...
TEST_F(ClientMetricsTest, IoDataSyncedMetricsExposeOnlyAttachedSource) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_NE(text.find("instance_id=\"" + stats->instance_id() + "\""),
                  std::string::npos);
        EXPECT_NE(text.find("semantics=\"data_synced_bucket_completions_v1\"} 1"),
                  std::string::npos);
        EXPECT_NE(text.find("mooncake_ssd_data_synced_buckets_completed_total 2\n"),
                  std::string::npos);
        EXPECT_NE(metric.summary_metrics().find("data_synced_buckets_completed_total=2"),
                  std::string::npos);
    }
    EXPECT_EQ(metric.ssd_write_ops.value(), 0);  // legacy per-key ops untouched
...
// Injected clocks make coverage, boundary, retention and idle-zero tests exact.
TEST(SsdKvIoMetricsTest, BoundariesOriginAndTimeBasedRetention) {
    ...
    EXPECT_EQ(after.completed_bytes[1], 48);  // evicted bytes stay cumulative
...
TEST(SsdKvIoMetricsTest, FrozenWireIsSparseExactAndIndependentOfDurableOps) {
    ...
    stats->RecordCompleted(SsdKvIoDirection::Read, (uint64_t{1} << 53) + 1);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        metric.serialize(text);
        EXPECT_NE(text.find(
            "mooncake_ssd_kv_io_window_info{schema_version=\"1\",instance_id=\"" +
        ...
        for (const auto* line : {
                 ...
                 "mooncake_ssd_kv_io_direction_available{direction=\"read\"} 1\n",
                 "mooncake_ssd_kv_io_direction_available{direction=\"write\"} 1\n",
...
TEST(SsdKvIoMetricsTest, UnsupportedWriteIsAbsentButIdleReadIsMeasuredZero) {
    auto stats = std::make_shared<SsdKvIoStats>(
        [] { return SsdKvIoStats::Clock::time_point{}; },
    ...
    for (int i = 0; i < 2; ++i) {
        std::string text;
        metric.serialize(text);
        EXPECT_NE(text.find("mooncake_ssd_kv_io_direction_available"
                            "{direction=\"read\"} 1\n"), std::string::npos);
        EXPECT_NE(text.find("mooncake_ssd_kv_io_direction_available"
                            "{direction=\"write\"} 0\n"), std::string::npos);
        EXPECT_NE(text.find("mooncake_ssd_kv_io_completed_bytes_total"
                            "{direction=\"read\"} 0\n"), std::string::npos);
        EXPECT_EQ(text.find("mooncake_ssd_kv_io_completed_bytes_total"
                            "{direction=\"write\"}"), std::string::npos);
        EXPECT_EQ(text.find("mooncake_ssd_kv_io_observation_losses_total"
                            "{direction=\"write\"}"), std::string::npos);
    }
    stats->RecordCompleted(SsdKvIoDirection::Read, 8);
    std::string text;
    metric.serialize(text);
    EXPECT_NE(text.find("bucket_id=\"0\",direction=\"read\"} 8\n"), std::string::npos);
    EXPECT_EQ(text.find("bucket_id=\"0\",direction=\"write\"}"), std::string::npos);
}
...
TEST(SsdKvIoMetricsTest, SerializationUsesOneSnapshotClockRead) {
    ...
    std::string text;
    metric.serialize(text);
    EXPECT_EQ(ticks, 3);  // construction, observation, snapshot
...
TEST_F(ClientMetricsTest,
       StorageWindowUsesCompletedBucketsWithoutResettingTotals) {
...
TEST_F(ClientMetricsTest,
       StorageWindowAggregatesThreadsAndKeepsDirectionsSeparate) {
    ...
    EXPECT_NE(prometheus.find("mooncake_storage_io_completed_bytes_total"
                              "{path=\"ssd_read\"} 1638400"),
              std::string::npos);
}
